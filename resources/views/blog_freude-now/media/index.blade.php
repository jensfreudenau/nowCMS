@push('meta_after')
    ...
    <link rel="canonical" href="https://{{Config::get('domains.name.freude_now_blog_domain')}}/archive">
            ...
            <div class="p-5 sm:p-8">
                <div class="columns-1 gap-5 sm:columns-2 sm:gap-8 md:columns-3 lg:columns-3 [&>img:not(:first-child)]:mt-8">
                    @foreach($contents as $content)
                        @if(Str::before($content->website, '.') === Str::before(config('domains.name.freude_now_blog_domain', env('APP_BASE_DOMAIN')), '.'))
